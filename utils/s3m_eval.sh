...
python3 ShapeletEvaluation.py -tr $INPUT -te $TEST -i $OUTPUT_JSON -o $OUTDIR -n $IDENTIFIER -l $LABEL_INDEX -k $KEEP  # test data
python3 ShapeletEvaluation.py -tr $INPUT -te $INPUT -i $OUTPUT_JSON -o $OUTDIR -n $IDENTIFIER -l $LABEL_INDEX -k $KEEP  # train data, the training set (-tr) and test set (-te) parameters are pointed to the same input data 
